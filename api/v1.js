...
var projectBySlug = (slug) => projects.find((project) => project.slug === slug);
...
var jsonError = (code, status, message, hint) => json(errorBody(code, status, message, hint), {
  status,
  cacheControl: NO_CACHE
});
var noContent = () => new Response(null, {
  status: 204,
  headers: {
...
var guardMethod = (request) => {
  if (request.method === "OPTIONS")
    return noContent();
  if (READ_METHODS.includes(request.method))
    return null;
  const response = jsonError("method_not_allowed", 405, `${request.method} is not supported. This API is read-only.`, `Use one of: ${READ_METHODS.join(", ")}.`);
  response.headers.set("Allow", READ_METHODS.join(", "));
  return response;
};
var guardJsonAccept = (request) => {
  if (negotiate(request.headers.get("accept"), [JSON_TYPE]).mediaType) {
    return null;
  }
  return jsonError("not_acceptable", 406, "This endpoint can only produce application/json.", "Send `Accept: application/json` or omit the header.");
};
...
// shared/api.ts
var collectionMeta = (count, total, category) => category ? { count, total, category } : { count, total };
var apiIndex = () => ({
  ...
  endpoints: [
    ...
    },
    {
      ...
      description: "Every shipped project. Supports ?category=Web3|AI|FullStack and ?limit=N."
    },
    {
...
var filterProjects = (url) => {
  ...
  if (category !== null) {
    if (!PROJECT_CATEGORIES.includes(category)) {
      return {
        projects: [],
        error: jsonError("bad_request", 400, `\`category\` must be one of: ${PROJECT_CATEGORIES.join(", ")}.`, "Retry with a supported category, or omit the parameter.")
      };
    }
    selected = selected.filter((project) => project.categories.includes(category));
  }
  if (limitRaw !== null) {
    ...
    if (!Number.isInteger(limit) || limit < 1 || limit > 100) {
      return {
        projects: [],
        error: jsonError("bad_request", 400, "`limit` must be an integer between 1 and 100.", "Retry with `?limit=10`, or omit the parameter to receive every project.")
      };
    }
...
var notFoundResource = (pathname) => jsonError("not_found", 404, `No API resource at "${pathname}".`, `List every endpoint at ${absolute(API_BASE)}.`);
var handleApiRequest = (request, pathnameOverride) => {
  ...
  const segments = pathname.slice(API_BASE.length).split("/").filter((segment) => segment.length > 0);
  const respond = (response) => stripBodyForHead(request, response);
  if (segments.length === 0)
    return respond(json(apiIndex()));
  ...
  switch (resource) {
    ...
    case "projects": {
      if (id) {
        ...
        if (!project) {
          return respond(jsonError("not_found", 404, `No project with slug "${id}".`, `List every available slug at ${absolute(`${API_BASE}/projects`)}.`));
        }
      ...
      return respond(json({
        ...
        meta: collectionMeta(result.projects.length, projects.length, result.category)
      }));
    }
    case "experience":
      if (id)
...
var RESOURCE_PARAM = "__resource";
var ID_PARAM = "__id";
var PATH_PARAM = "__path";
var apiPathnameFor = (request) => {
  const url = new URL(request.url);
